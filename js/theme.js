...
document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
        ...
        document.addEventListener('input', (e) => {
            if (e.target && e.target.type === 'range') {
                ...
                if (e.target.id !== 'val-theme-preset' && e.target.id !== 'val-popup-anim') {
                    ...
                    resetPresetToCustom(isShadowSlider);
                }
        ...
        if (drawerEl) {
            ...
            drawerEl.addEventListener('change', (e) => {
                if (e.target.type === 'color' || e.target.type === 'checkbox') {
                    const isShadowControl = e.target.classList.contains('shadow-switch') || e.target.classList.contains('s-c');
                    if (e.target.id !== 'val-theme-preset' && e.target.id !== 'val-popup-anim') {
                        resetPresetToCustom(isShadowControl);
                    }
                    updateLiveVariables(true);
                    if (e.target.type === 'color') setTimeout(() => { colorPickerLock = false; }, 150);
                }
            });
        ...
        if (btnLayoutList) btnLayoutList.onclick = () => { setLayoutMode('list'); resetPresetToCustom(false); }; 
        if (btnLayoutGrid) btnLayoutGrid.onclick = () => { setLayoutMode('grid'); resetPresetToCustom(false); };
        ...
        if (btnImport) {
            btnImport.addEventListener('click', (e) => { 
                ...
                if (code) { unpackConfig(code); resetPresetToCustom(false); } 
            });
        }
